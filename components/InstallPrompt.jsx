...
export default function InstallPrompt() {
    ...
    useEffect(() => {
        ...
        const isSafariBrowser = /safari/i.test(navigator.userAgent) && !/crios|fxios|opios|chrome/i.test(navigator.userAgent);
        if (isIos && isSafariBrowser && !window.navigator.standalone) {
                        ...
                        <div className="flex gap-2 mt-3">
                            <button onClick={handleDismiss}
                                className="flex-1 py-2 text-xs font-semibold text-white/45
                                           hover:text-white/70 transition-colors rounded-lg bg-white/5 hover:bg-white/8">
                                Not now
                            </button>
