...
export const contactApi = {
    ...
    getContacts: async ({ status, priority, search = '', page = 1, limit = 20 } = {}) => {
        try {
            const params = new URLSearchParams();
            if (status) {
                params.append('status', status);
    ...
    // Update a contact (status, priority, notes, reply)
...
export const notificationApi = {
    ...
    markAllAsRead: async () => {
        try {
            ...
            console.error('Failed to mark all messages as read:', error.message);
