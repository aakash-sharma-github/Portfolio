 ...
 *   clearCacheByPattern(prefix)      → Promise<void>
 *   buildCacheKey(namespace, params) → string
 */
...
export async function clearCache(key) {
    ...
    if (client) {
        try {
            if (key) { await client.del(key); } else { await client.flushdb(); }
            return;
        } catch (err) {
...
/**
 * Build a consistent, canonical cache key.
 * e.g. buildCacheKey('github', { login: 'aakash' }) → 'github:login=aakash'
 */
...
export const TTL = {
    ...
    GITHUB: 21600, // 6 hours   — GitHub stats
    GITHUB_SHORT: 1800,  // 30 min    — GitHub stats on error (retry sooner)
};